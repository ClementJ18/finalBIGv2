...
class AdaptiveDeltaBlock:
    ...
    def read(io_stream, vec_index, bits):
        result = AdaptiveDeltaBlock(
            vector_index=vec_index, block_index=read_ubyte(io_stream), delta_bytes=[]
        )
...
class AdaptiveDeltaData:
    ...
    def read(io_stream, channel, bits):
        result = AdaptiveDeltaData(
            initial_value=read_channel_value(io_stream, channel.type), bit_count=bits
        )
...
class AdaptiveDeltaMotionAnimationChannel:
    ...
    @staticmethod
    def read(io_stream, channel, bits):
        result = AdaptiveDeltaMotionAnimationChannel(scale=read_float(io_stream), data=None)
...
class TimeCodedBitDatum:
    ...
    @staticmethod
    def read(io_stream):
        result = TimeCodedBitDatum(time_code=read_ulong(io_stream))
    ...
    def read_time_coded_data(self, io_stream):
        ...
        for _ in range(self.num_time_codes):
            datum = TimeCodedDatum(
                time_code=read_short(io_stream), interpolated=True
            )  # non interpolation is not supported here
            result.append(datum)
...
class CompressedAnimation:
    def __init__(
        self,
        header=None,
        time_coded_channels=None,
        adaptive_delta_channels=None,
        time_coded_bit_channels=None,
        motion_channels=None,
    ):
        self.header = header
        self.time_coded_channels = time_coded_channels if time_coded_channels is not None else []
        self.adaptive_delta_channels = (
            adaptive_delta_channels if adaptive_delta_channels is not None else []
        )
        self.time_coded_bit_channels = (
            time_coded_bit_channels if time_coded_bit_channels is not None else []
        )
    ...
    def validate(self, context, w3x=False):
        ...
        if len(self.header.name) > STRING_LENGTH:
            logging.error(f"animation name exceeds max length of: {STRING_LENGTH}")
            return False
        if len(self.header.hierarchy_name) > STRING_LENGTH:
            logging.error(f"animation hierarchy name exceeds max length of: {STRING_LENGTH}")
            return False
        return True

...
def skip_unknown_chunk(io_stream, chunk_type, chunk_size):
    logging.warning(f"unknown chunk_type in io_stream: {hex(chunk_type)}")
    io_stream.seek(chunk_size, 1)


def read_chunk_array(io_stream, chunk_end, type_, read_func):
    result = []
    ...
    while io_stream.tell() < chunk_end:
        ...
        if chunk_type == type_:
            result.append(read_func(io_stream, subchunk_end))
        else:
